...
// NOTE: If you still get an import error here, verify this relative path matches your folder structure!
    ...
    <div class="page-container">
      <header class="ph">
        <div>
          ...
          <div class="ps">{{ totalCount }} registered customers</div>
        </div>
...
export class CustomersComponent implements OnInit {
  ...
  loadCustomers() {
    ...
    let url = `${environment.apiUrl}/Customer?page=${this.page}&pageSize=${this.pageSize}`;
    
    // Append filters to API request if they exist
    if (this.search) {
    ...
    this.http.get<any>(url).subscribe({
      next: (res) => {
        // Map paginated server response correctly
  ...
  applyFilter() {
    ...
    this.loadCustomers(); // Reload from server
  }
  ...
  save() {
    ...
    req.subscribe({
      next: () => { 
        this.closeModal(); 
        this.saving = false; 
        this.loadCustomers(); // Reload current page to see updates
